...
void delete(List L){
    ...
    if(L->index == 0) {
        ...
        N = L->curser;
        L->curser->after->before = L->curser->before;
        L->curser->before->after = L->curser->after;
        L->index = -1;
        L->curser = NULL;
        L->length--;
        freeNode(&N);
...
void printList(FILE* out, List L) {
   ...
   if( L==NULL ){
      printf("Print Error: calling printList() on NULL List reference\n");
      exit(1);
   }
